...
n_theta=[25,45,27,17,15,19,20,19,18,51,43,60,24] #number of measurements files for each folder
...
"""
# for k in range(12,13):#len(foldername)):
...
for k in range(0,1):#len(foldername)):
    ...
    for i in range(len(roi[:,0])):
        ...
        axs[2].axvline(a,color='y', ls = '--')
        axs[2].axvline(b,color='k', ls = '--')
        if (npeaks[i][0]>1 or npeaks[i][1]>1):
...
#                 boundm = [[0.2,0.,0.,-2.,abs(roi[y][5]-xabsmax)-4, abs(roi[y][5]-xabsmax)+2,0.01,0.1,0.1],[1.5,1.5,0.4,2.,abs(roi[y][1]-xabsmax),abs(roi[y][1]-xabsmax)-2, 1.,1.5,2.]]
#             if(roi[y][3]>1):    
#                 P0m = [1.,0.3,0.1, 0., abs(roi[y][5]-xabsmax)-2, abs(roi[y][5]-xabsmax)+8, 0.5, 0.5, 0.5]
...
#             aus = (stack[roi[y][0],xabsmax,:].copy()+stack[roi[y][0],xabsmax-1,:].copy()+stack[roi[y][0],xabsmax+1,:].copy())/3
#             if (np.amax(aus)>150):
#                 zmin1 = np.where(aus==np.amin(aus))[0][0]
#                 for i in range(5):
#                     if (zmin1+i<len(aus)):
...
#             ax1.set_title(foldername[k] +'-Line ' +str("%0d"%(roi[0][0]+y)))
#             ax1.plot((stack[roi[y][0],xabsmax,:]+stack[roi[y][0],xabsmax-1,:]+stack[roi[y][0],xabsmax+1,:])/3, "k-")
#             ax1.axvline(zmin1,color='r')
#             ax1.axvline(zmin2,color='b')
#             #print(zmin1,zmin2)
#             for z in range(len(stack[0,0,:])):
#                 data=np.zeros((roi[y][2]-roi[y][1]+1,2))
#                 data[:,0] =  np.arange(roi[y][1],roi[y][2]+1)
...
#                 ax.set_title(foldername[k] +'-Line ' +str("%0d"%(roi[0][0]+y))+'_theta'+str("%0d"%(z)))
#                 ax.plot(data[:,0],data[:,1], "ko")
#                 if(roi[y][3]==0 or np.amax(data[(roi[y][1]-xabsmax):-2])<3*bg1):
#                     boundm[1][1] = 1e-8
#                     P0m[1]=0.
#                     boundm[1][2] = 1e-8
#                     P0m[2]=0.
...
#                         boundm[1][2] = 0.1
#                         P0m[2]=0.09
#                 if(roi[y][4]==0 or np.amax(data[4:(roi[y][2]-xabsmax)])<3*bg1):
#                     boundp[1][1] = 1e-8
#                     P0p[1]=0.
#                     boundp[1][2] = 1e-8
#                     P0p[2]=0.
